...
func NewAssessmentRepository(db *gorm.DB, log *zap.SugaredLogger) *AssessmentRepository {
	return &AssessmentRepository{
		Base: NewBaseRepository[models.Assessment](db, log.Named("assessment"), "assessments"),
	}
}

// CreateAssessment creates a new assessment with structured data
func (r *AssessmentRepository) CreateAssessment(assessment *models.AssessmentSubmission) (uint, error) {
	log := r.Base.Log.With(
	...
	var userCount int64
	if err := r.Base.DB.Model(&models.User{}).Where("email = ?", assessment.UserEmail).Count(&userCount).Error; err != nil {
	...
	result := r.Base.DB.Where("id = ? AND user_email = ?", assessment.DeviceID, assessment.UserEmail).First(&device)
	if result.Error != nil {
		log.Errorw("Failed to find device", "error", result.Error)
		return 0, fmt.Errorf("device not found or doesn't belong to user: %w", result.Error)
	}

	// Update device last active time
	device.LastActive = time.Now()
	r.Base.DB.Save(&device)
	...
	// Save to database
	if err := r.Base.Create(&newAssessment); err != nil {
		return 0, err
	}
...
func (r *AssessmentRepository) saveStructuredResponses(assessmentID uint, responses models.JSON) error {
	...
	if len(questionResponses) > 0 {
		return r.Base.DB.CreateInBatches(questionResponses, 100).Error
	}
...
func (r *AssessmentRepository) GetMetricsCorrelation(userID, symptomKey, metricKey string) ([]CorrelationDataPoint, error) {
	...
	err := r.Base.DB.Raw(query, userID, symptomKey, metricKey, symptomKey).Scan(&result).Error
	if err != nil {
		r.Base.Log.Errorw("Error in correlation query", "error", err)
		return nil, fmt.Errorf("database error: %w", err)
	}

	r.Base.Log.Infow("Retrieved correlation data",
		"user_id", userID,
		"symptom", symptomKey,
		"metric", metricKey,
...
func (r *AssessmentRepository) GetMetricsTimeline(userID, symptomKey, metricKey string) ([]TimelineDataPoint, error) {
	...
	err := r.Base.DB.Raw(query, userID, symptomKey, metricKey, symptomKey).Scan(&result).Error
	if err != nil {
		r.Base.Log.Errorw("Error in timeline query", "error", err)
		return nil, fmt.Errorf("database error: %w", err)
	}

	// Log the results for debugging
	r.Base.Log.Infow("Retrieved timeline data",
		"user_id", userID,
		"symptom", symptomKey,
		"metric", metricKey,
		"points_count", len(result))
...
func (r *AssessmentRepository) extractAndSaveMetrics(assessmentID uint, metadata json.RawMessage) error {
	...
	if len(metrics) > 0 {
		return r.Base.DB.CreateInBatches(metrics, 100).Error
	}
