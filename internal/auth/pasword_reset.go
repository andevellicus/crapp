...
func (s *AuthService) GeneratePasswordResetToken(email string) (string, error) {
	...
	_, err := s.repo.Users.GetUser(email)
	if err != nil {
		return "", fmt.Errorf("user not found: %w", err)
	}

	// Create a reset token (valid for 30 minutes)
	token, err := s.repo.PasswordTokens.Create(email, 30)
	if err != nil {
...
func (s *AuthService) ValidatePasswordResetToken(tokenStr string) (string, error) {
	token, err := s.repo.PasswordTokens.ValidatePasswordResetToken(tokenStr)
	if err != nil {
...
func (s *AuthService) ResetPassword(tokenStr string, newPassword string) error {
	...
	if err := s.repo.Users.UpdateUserPassword(userEmail, hashedPassword); err != nil {
		return fmt.Errorf("failed to update password: %w", err)
	}

	// Mark token as used
	if err := s.repo.PasswordTokens.MarkTokenAsUsed(tokenStr); err != nil {
